...
def test_eda(churn_library_object):
    ...
    for fp in churnlib.images_path.iterdir():
        if fp.name in image_names: fp.rename(backup_path/f"backup_{fp.name}")
...
def test_perform_feature_engineering(churn_library_object):
    ...
    log_assert(
        set(churn_library_object.X.columns.values) == set(churnlib.consts.quant_columns),
        "Issue in selection of numerical columns"
    )
...
def test_train_models(churn_library_object):
    ...
    models_path_list = [fp.name for fp in churnlib.models_path.iterdir() if fp.is_file()]
    for key,val in churnlib.consts.model_names.items():
...
def test_results(churn_library_object):
    ...
    results_path_list = [fp.name for fp in churnlib.images_path.iterdir() if fp.is_file()]
    for key,val in churnlib.consts.result_plots.items():
